...
Nhờ vậy:
- **Backend** không cần viết lại các đoạn `if (!hasPermission) throw forbiddenException()` nữa, vì đã có `AuthorizeApiMiddleware` tự động kiểm tra.  
      ...
      policies: [
        (context) =>
          new SimpleRolePolicy({
            permission: permissions.CreateMessage.id,
          }),
      ],
    },
  }),
      ...
      policies: [
        (context) =>
          new RolePermissionPolicy({
            permission: permissions.ReadChannel.id,
