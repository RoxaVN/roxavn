...
mkdir test-module
cd test-module
npx -p @roxavn/cli roxavn create module
```
...
```bash
npx roxavn create module:convention
```
...
11. Lúc này bạn vào trang `http://[DEV_SERVER]/admin/apps` thì sẽ chưa thấy module mới. Vì module `@roxavn/module-project` có thêm những permissions mới, nên cần cập nhật những permissions này cho user admin. Module `@roxavn/module-project` đã export sẵn script để cập nhật quyền cho admin qua seed hook, nên chỉ cần chạy lại `hook`
...
```bash
npx roxavn create access
```

2. Tạo template api cho đối tượng `task`

```bash
npx roxavn create api
...
import { ResourceService, useApi } from '@roxavn/core/server';
import { taskApi, scopes } from '../../base/index.js';

@useApi(taskApi.create)
export class CreateTaskService extends ResourceService {
  handle(request: InferApiRequest<typeof taskApi.create>) {
    return super.create('task', request);
  }
}

@useApi(taskApi.getMany)
export class GetManyTasksService extends ResourceService {
  handle(request: InferApiRequest<typeof taskApi.getMany>) {
    return super.getMany('task', request);
  }
}
```

Bạn có thể tự khám phá thêm chế độ tạo các loại api khác `api:delete`, `api:update`, `api:getOne`, ...
...
Sau khi chạy xong, script sẽ tự động đồng bộ kiểu với các bảng trong database. Bạn có thể kiểm tra ở thư mục `src/server/schemas`
...
import { resourceUI, ResourceList } from "@roxavn/core/web";
...
2. Tạo script để thêm permission mới cho `admin` bằng cách tạo seed hook

```bash
npx roxavn create hook
```

Xem nội dung file `src/hook/seed.ts`

```ts
import { BaseSeedHook } from '@roxavn/core/server';
import { roles } from '../../base/index.js';
import { serverModule } from '../module.js';

@serverModule.useHook()
export class SeedHook extends BaseSeedHook {
  async handle() {
...
3. Chạy seed hook để cập nhật role mới
...
1. Để tạo 1 web page hỗ trợ SEO (cần server side render) chạy lệnh `npx roxavn create web:route`. Nhập thông tin như sau
