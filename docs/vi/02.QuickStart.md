...
import { BaseService, resourceService, useApi } from '@roxavn/core/server';
...
export class CreateTaskService extends BaseService {
  handle(request: InferApiRequest<typeof taskApi.create>) {
    return resourceService.create('task', request);
  }
}

@useApi(taskApi.getMany)
export class GetManyTasksService extends BaseService {
  handle(request: InferApiRequest<typeof taskApi.getMany>) {
    return resourceService.getMany('task', request);
  }
}
```
