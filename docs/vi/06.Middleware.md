...
Trong RoxaVN, mọi API được gọi thông qua một **API pipeline**, nơi mỗi middleware có thể can thiệp, xác thực, hoặc biến đổi request/response.  
Hai khái niệm quan trọng trong pipeline là **ApiContext** và **MiddlewareService**.

### ApiContextState

`ApiContextState` chứa toàn bộ trạng thái của request đang được xử lý. Đây là phần dữ liệu "mềm" mà middleware hoặc API handler có thể đọc, ghi hoặc sửa đổi trong suốt vòng đời của request.

```ts
export interface ApiContextState<T extends Api = Api> {
...
`ApiContext` là đối tượng truyền qua toàn bộ pipeline xử lý API.
Mỗi middleware nhận `ApiContext` và có thể đọc, ghi, hoặc chặn luồng xử lý.

```ts
export interface ApiContext {
  request: Request; // Đối tượng request gốc (từ Node.js, ...)
  state: ApiContextState; // Trạng thái mở rộng của request
...
```ts
@useApiMiddleware()
export class ValidateApiRequestMiddleware extends MiddlewareService {
  priority: number;

  async handle(context: ApiContext, next: () => Promise<void>) {
...
#### 2. ValidateApiRequestMiddleware
...
|   2️⃣    | `ValidateApiRequestMiddleware` | Kiểm tra đầu vào có hợp lệ với schema không |
|   3️⃣    | `AuthenticateApiMiddleware`    | Xác thực user từ token                      |
|   4️⃣    | `AuthorizeApiMiddleware`       | Kiểm tra quyền truy cập qua policy          |
