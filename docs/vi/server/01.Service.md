...
Để tạo một resource mới, bạn chỉ cần gọi `resourceService.create()` và liên kết API tương ứng thông qua decorator `@useApi()`:

```ts
@useApi(storeApi.create)
export class CreateStoreService extends BaseService {
  handle(request: InferApiRequest<typeof storeApi.create>) {
    return resourceService.create('store', request);
  }
}
```
...
export class UpdateStoreService extends BaseService {
  handle(request: InferApiRequest<typeof storeApi.update>) {
    return resourceService.update('store', request);
  }
}
```
...
export class UpsertStoreService extends BaseService {
  handle(request: InferApiRequest<typeof storeApi.upsert>) {
    return resourceService.upsert('store', request, ['id']);
  }
}
```
...
export class DeleteStoreService extends BaseService {
  handle(request: InferApiRequest<typeof storeApi.delete>) {
    return resourceService.delete('store', request);
  }
}
```
...
```ts
@injectable()
export class GetFileStorageService extends BaseService {
  handle(request: { id: string }) {
    return resourceService.getOne('fileStorage', request);
  }
}
```
...
export class GetProjectsService extends BaseService {
  handle(request: InferApiRequest<typeof projectApi.getMany>) {
    return resourceService.getMany('project', request);
  }
}
```
...
export class GetAllStatisticConfigsService extends BaseService {
  handle(request: InferApiRequest<typeof statisticConfigApi.getAll>) {
    return resourceService.getAll('statisticConfig', request);
  }
}
```
