...
`useResourceChangeJob()` là một tiện ích mở rộng (wrapper) của `useJob()`, được thiết kế để tự động kích hoạt khi một **resource (tài nguyên)** trong hệ thống có sự thay đổi — như thêm mới **(insert)**, cập nhật **(update)**, hay xóa **(delete)**.
